...
class ReplayBuffer:
    ...
    def reset_buffer(self):
        self.buffer = {'obs_n': np.empty([self.batch_size, self.episode_limit, self.N, self.obs_dim]),
                       ...
                       # 'mean_n': np.empty([self.batch_size, self.episode_limit, self.N, self.action_dim]),
                       # 'std_n': np.empty([self.batch_size, self.episode_limit, self.N, self.action_dim]),
    ...
    def store_transition(self, episode_step, obs_n, s, v_n, a_n, a_logprob_n, r_n, done_n):
        ...
        # self.buffer['std_n'][self.episode_num][episode_step] = std
        # self.buffer['mean_n'][self.episode_num][episode_step] = mean
