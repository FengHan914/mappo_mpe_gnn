...
class MutiAgentEnv:
    ...
    def render(self, mode='human'):
        ...
        #     target_center = self.scene.target_area  # 例如: [110, 30]
        #     target_radius = self.scene.target_radius  # 例如: 5.0
        #
        #     # 创建一个表示目标区域的圆形 patch
        #     # 您可以自定义颜色、线条样式等
        #     target_zone_circle = patches.Circle(
        #         tuple(target_center),  # 圆心位置，确保是元组
        #         target_radius,  # 半径
        #         edgecolor='blue',  # 边界颜色 (例如蓝色)
        #         facecolor='none',  # 无填充色
        #         linestyle='--',  # 虚线样式
        #         linewidth=1.5,  # 线宽
        #         label='Evader Target Zone'  # 图例标签 (可选)
        #     )
        #     # 将圆形添加到绘图区域
        #     ax.add_patch(target_zone_circle)
        ...
        # # 2. 绘制逃逸者出生区域
        # # 注意: 这个区域是根据 entrap.py 中的 reset_world 硬编码的，如果修改了场景，这里也需要同步修改
        # evader_spawn_rect = patches.Rectangle(
        #     (10, 100), 10, 10,  # (x, y), width, height
        #     edgecolor='#FF6600', facecolor='#FFC18E', alpha=0.4,
        #     linewidth=1.5, linestyle=':', label='逃逸者出生区'
        # )
        # ax.add_patch(evader_spawn_rect)
        # ax.text(15, 112, '逃逸者出生区', ha='center', color='#CC5200', fontsize=10)
        #
        # # 3. 绘制追捕者出生区域
        # # 同样是基于 entrap.py 硬编码
        # pursuer_spawn_rect = patches.Rectangle(
        #     (10, 1), 70, 22,  # 一个大致覆盖所有追捕者出生点的区域
        #     edgecolor='#D62728', facecolor='#FF9999', alpha=0.4,
        #     linewidth=1.5, linestyle=':', label='追捕者出生区'
        # )
        # ax.add_patch(pursuer_spawn_rect)
        # ax.text(45, 25, '追捕者出生区', ha='center', color='#A31F20', fontsize=10)
        # ----------------------------------------------------------- #
        # ----------------------------------------------------------- #
        #  # 创建一个红色虚线圆形，圆心位置为 [150, 100]，半径为 10
        # circle = patches.Circle((150, 100), radius=10, edgecolor='red', facecolor='none', linestyle='--')
        # # 将圆形添加到绘图区域
        # ax.add_patch(circle)
        # 显示标签
        x1 = [120]
